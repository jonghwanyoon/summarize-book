...
2. **디렉토리 준비**: `mkdir -p ./00_INBOX/BOOK ./ATTACHMENTS`
3. **페이지 크롤링**: 전달된 url의 문서를 playwright tool로 읽어서 아래 정보 추출
...
4. **분류**: Read tool로 `categories.md` 파일을 읽고, 추출한 카테고리 정보를 8개 그룹 중 하나로 매핑
5. **가이드라인 로드**: Read tool로 분류된 그룹에 해당하는 `category-{그룹}.md` 파일을 읽기
6. **내용 유추 및 요약**: 가이드라인의 지시에 따라 요약 생성 (아래 출력 포맷 참조)
7. **이미지 저장**: 표지 이미지를 `curl -o "./ATTACHMENTS/{유니크제목}_cover.png" "{이미지URL}"` 로 다운로드
   - 유니크 제목: 한글 제목에서 공백을 `-`로, 특수문자 제거 (예: `클린-코드`)
8. **마크다운 생성**: Write tool로 `./00_INBOX/BOOK/{한글제목}.md` 파일 생성
...
- url에 접근할 때는 반드시 playwright tool을 사용해. fetch tool을 사용하면 접근이 안될 수 있어
- `./00_INBOX/BOOK` 및 `./ATTACHMENTS` 디렉토리가 없으면 생성할 것
- 이미지는 하나도 누락 없이 저장하고 문서에 포함할 것
- 가이드라인 파일(categories.md, category-*.md)은 Read tool로 읽을 것
